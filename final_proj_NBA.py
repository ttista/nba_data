...
def process_attendance(command):
    ...
    if len(command_split) != 2:
        ...
        if nickname not in nba_team_list.keys():
            return 'invalid command'

        else:
            statement = 'SELECT  Point_Differential, Team_Attendance '
            statement += 'FROM Attendance '
    ...
    score = []
    amt = []
    layout = go.Layout(title = '{} Point Differential and Attendance'.format(nickname),
...
def interactive_prompt():
    ...
    print()
    print("The NBA: Scores and Attendance")
    print()
